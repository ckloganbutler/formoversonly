...
if(isset($_GET['clock'])){
    ...
    if($_GET['clock'] == 'in'){
        ...
        if(mysql_num_rows($out) > 0){
            ...
            mysql_query("UPDATE fmo_users_employee_timeclock SET timeclock_clockout='".mysql_real_escape_string($date_out)."', timeclock_hours='".mysql_real_escape_string($worked)."', timeclock_complete=1 WHERE timeclock_id='".mysql_real_escape_string($out['timeclock_id'])."'") or die(mysql_error());
            echo $worked;
        } else {
