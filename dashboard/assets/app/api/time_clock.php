...
if(isset($_GET['admin']) && $_GET['admin'] == 'trl'){
    ...
    $findEmployees = mysql_query("SELECT timeclock_id, timeclock_clockin, timeclock_clockout, timeclock_hours, timeclock_timestamp FROM fmo_users_employee_timeclock WHERE timeclock_user='".mysql_real_escape_string($_GET['uuid'])."' ORDER BY timeclock_timestamp DESC");
    ...
    $iDisplayStart = intval($_REQUEST['start']);
    $sEcho = intval($_REQUEST['draw']);
    $findEmployees = mysql_query("SELECT timeclock_id, timeclock_clockin, timeclock_clockout, timeclock_hours, timeclock_timestamp FROM fmo_users_employee_timeclock WHERE timeclock_user='".mysql_real_escape_string($_SESSION['uuid'])."' ORDER BY timeclock_timestamp DESC");
    $iTotalRecords = mysql_num_rows($findEmployees);
