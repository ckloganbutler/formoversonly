...
if(isset($_GET['setting'])){
    if($_GET['setting'] == 'su'){
        ...
        mysql_query("INSERT INTO fmo_locations_storages_contracts_items (item_contract_token, item_company_token, item_user_token, item_location_token, item_item, item_desc, item_qty, item_cost, item_total, item_taxable, item_commission, item_redeemable, item_discount, item_percent, item_prepay, item_adder) VALUES (
            '".mysql_real_escape_string($contract)."',
            '".mysql_real_escape_string($_SESSION['cuid'])."',
            '".mysql_real_escape_string($user)."',
            '".mysql_real_escape_string(stlc($storage))."',
        ...
        if($ol < 0){
            $due = "Credit";
            $new = number_format($ol * -1, 2);
        } else {$due = "Due"; $new = number_format($ol, 2); }
        timeline_str($user, $contract, $_SESSION['uuid'], "Charge", "<strong>$".number_format($_POST['deposit'], 2)." deposit charge</strong> for <strong>".date('m/d/Y', strtotime('today'))."</strong> was systematically added. <strong>(Unit #".$unit['storage_unit_name'].")</strong> [By: <strong>".name($_SESSION['uuid'])."</strong>] <button style='width: 150px;' class='pull-right btn btn-xs default red-stripe'>".$due.": <strong>$".$new."</strong></button><button style='width: 150px;' class='pull-right btn btn-xs default red-stripe'>Charge: <strong>$".number_format($_POST['deposit'], 2)."</strong></button> <button style='width: 150px;' class='pull-right btn btn-xs default red-stripe'>&nbsp;</button>");
        ...
        timeline_str($user, stlc($storage), $_SESSION['uuid'], "Charge", " <strong>$".number_format($_POST['rent'], 2)." rent charge</strong> for <strong>".date('m/d/Y', strtotime('today'))."</strong> was added systematically. <strong>(Unit #".$unit['storage_unit_name'].")</strong> [By: <strong>".name($_SESSION['uuid'])."</strong>] <button style='width: 150px;' class='pull-right btn btn-xs default red-stripe'>".$due.": <strong>$".$new."</strong></button><button style='width: 150px;' class='pull-right btn btn-xs default red-stripe'>Charge: <strong>$".number_format($_POST['rent'], 2)."</strong></button> <button style='width: 150px;' class='pull-right btn btn-xs default red-stripe'>&nbsp;</button>");


        mysql_query("UPDATE fmo_locations_storages_contracts_items SET item_user_token='".mysql_real_escape_string($user)."' WHERE item_contract_token='".mysql_real_escape_string($contract)."'") or die(mysql_error());
        $bal2   = json_decode(file_get_contents('https://www.formoversonly.com/dashboard/assets/app/api/storage.php?type=inv_c&luid='.stlc($storage).'&uuid='.$user.''), true);
        ...
        if($_POST['amount'] > 0){
            ...
            $old  = number_format($bal);
            $bal  = number_format($retail - $amount + $fee, 2);
        ...
        if(!empty($storage) && !empty($user)){
            ...
            mysql_query("UPDATE fmo_users SET user_address='".mysql_real_escape_string($address)."', user_city='".mysql_real_escape_string($city)."', user_state='".mysql_real_escape_string($state)."', user_zip='".mysql_real_escape_string($zip)."', user_employer_dln='".mysql_real_escape_string($dln)."' WHERE user_token='".mysql_real_escape_string($user)."'");
        }

        $review_link = $googer->shorten("https://www.formoversonly.com/dashboard/assets/public/index.php?t=sTr&su=true&uuid=".$user);
        $msg        = "[".locationNickName(stlc($storage))."]\r\nThanks for moving in! Track your history here:\r\n".$review_link;
        _sendText(phone2($user), $msg);
    }
    if($_GET['setting'] == 'su_comment'){
        $user = $_GET['uuid'];
        $by   = $_SESSION['uuid'];
        $comment = $_POST['comment'];

        timeline_str($user, stlc($storage), $by, "Comment", " <strong>".name($by)."</strong> commented on <strong>".date('m/d/Y', strtotime('today'))."</strong>: <button style='width: 450px;' class='pull-right btn btn-xs default blue-stripe'><strong>".$comment."</strong></button>");

    }
    if($_GET['setting'] == 'alt_contact'){
        $user = $_GET['uuid'];
        $alt[0] = $_POST['fullname'];
        $alt[1] = $_POST['address'];
        $alt[2] = $_POST['phone'];
    ...
    if($_GET['setting'] == 'teller') {
        ...
        if(move_uploaded_file($_FILES['ticket']['tmp_name'], $uploadfile . $file_ext) && $file_ext != '.html'){
            $image = imagecreatefromjpeg($uploadfile . $file_ext);
            $exif = exif_read_data($uploadfile . $file_ext);
            $ort   = $exif['Orientation'];
            $width = $exif['COMPUTED']['Width'];
